...
export default function HeroSection() {
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      setActive((prev) => (prev + 1) % settings.heroSlides.length);
    }, 5000);
    return () => {if (intervalRef.current) clearInterval(intervalRef.current);};
  }, [settings.heroSlides.length]);

  if (!settings.heroSlides.length) return null;

  const slide = settings.heroSlides[active];

  return (
    <section className="relative w-full min-h-screen flex items-end overflow-hidden" aria-label={`Accueil ${settings.restaurantName}`}>
      {/* Background image */}
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 pb-20 md:pb-32 pt-32">
        ...
        style={{ transitionDelay: '200ms' }}>
          {[
          { value: settings.statsOrderPerDay, label: 'Commandes / jour' },
          { value: settings.statsDeliveryTime, label: 'Livraison moyenne' },
          { value: settings.statsRating, label: 'Note clients' }].
          map((stat, i) =>
